...
// C = U * L
void MmultUL(double **U, int nU, double **L, int nL, double **C, int nC) {
    assert(nC == nU && nU == nL);
    #pragma omp parallel for shared (nU, U, L, C) collapse(2) schedule(static) if (nU > 8)  
    for (int r = 0; r < nU; r++) {
        for (int c = 0; c < nU; c++) {
            double sum = 0;
...
// B = A^T (A is lower triangular matrix)
// B should be initialized as zeroed matrix
void LTMTranspose(double **A, int rA, int cA, double **B, int rB, int cB) {
    assert(rA == cB && cA == rB);
    for (int i = 0; i < rA; i++) {
        for (int j = 0; j < i; j++) {
            B[j][i] = A[i][j];
...
double GPR(int m, double rstar_l, double rstar_r) {
    ...
    OMPRinverse(U, n); // inv(U)

    wctduration = (chrono::system_clock::now() - wcts);
    printf("Matrix inverse of U %.3f ms\n", wctduration.count());
    wcts = chrono::system_clock::now();

    UTMTranspose(U, n, n, L, n, n); // inv(U) = inv(L)^T

#ifdef OPT1
    MmultUL(U, n, L, n, tmp1, n);
#else
    Mmult(U, n, n, L, n, n, tmp1, n, n);
#endif
    Mmult(tmp1, n, n, f, n, 1, tmp2, n, 1);
